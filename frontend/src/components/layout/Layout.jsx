...
import {
  ...
  MessageCircle, Settings, LogOut, Flame, BadgeCheck, Plus,
  ChevronDown, ChevronUp, MoreHorizontal, Sparkles
} from 'lucide-react';
import { suggestedAccounts, currentUser, formatNumber, notifications } from '../../data/mockData';
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-[280px] z-40 flex flex-col border-r border-white/[0.06]" style={{ background: 'rgba(5,5,10,0.95)', backdropFilter: 'blur(40px)' }}>
      ...
      <div className="p-4 border-t border-white/[0.06]">
        <p className="text-[10px] text-white/20 text-center leading-relaxed">© 2025 KdM · Terms · Privacy · Safety</p>
      </div>
    </aside>
  );
};
      ...
      <div className="flex items-center gap-2">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              ...
              <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-[#ff0050]" />
            </motion.button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="end" className="w-80 glass-strong rounded-2xl p-2" style={{ background: 'rgba(15,15,25,0.95)', backdropFilter: 'blur(40px)', border: '1px solid rgba(255,255,255,0.08)' }}>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56 rounded-2xl p-2" style={{ background: 'rgba(15,15,25,0.95)', backdropFilter: 'blur(40px)', border: '1px solid rgba(255,255,255,0.08)' }}>
            <DropdownMenuItem className="flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer focus:bg-white/[0.05]" onClick={() => {}}>
